 ...
 * as a <i>segment</i>. This class uses
 * {@code Integer.MAX_VALUE - }{@link LongArray#JVM_OVERHEAD} as the
 ...
 * {@link LongArray#JVM_OVERHEAD} is {@code 16} and is needed because VMs
 ...
 * precaution.
 * <p>
 * <b>NOTE:</b> the code has been tested for small values of \(k\) (<i>i.e.</i>,
 * for \(k \lt 16\)) when the base is \(4\). In general, \(64 \times base^k\)
 * bits are required to be stored in memory.
 * <p>
 * TODO: further test this class when the base is \(4\) and \(k\) is \(16\) or
 *  larger.
 *
...
public class LongArray {
     ...
     */
    public LongArray(byte n, byte k, byte base) {
        this.n = n;
        this.k = k;
        this.base = base;
     ...
     * specified. This should <em>only</em> be used for testing purposes to
     * ensure there doesn't exist a bug in dividing the arrays and/or
     * accessing/setting specific indices.
     *
     * @param size the number of cells to create
     */
    public LongArray(long size) {
        System.out.println("WARNING NOTE: the LongArray constructor used is " +
                "dedicated only for testing purposes.");
        this.SIZE = size;
        if (SIZE <= 0) {
            throw new RuntimeException("Size cannot be less than 1");
        }
     ...
     * false} otherwise
     */
    public boolean equalsTo(LongArray passed) {
